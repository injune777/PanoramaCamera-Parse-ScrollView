...
@interface PeoplesVCon ()<UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate, UIGestureRecognizerDelegate>
//背景照
@property (weak, nonatomic) IBOutlet UIImageView *backgroundImageView;
//頭照
@property (weak, nonatomic) IBOutlet UIImageView *headPictureImageView;
//customer cell
@property(nonatomic, strong) PeoplesTBViewCell *cell;
...
- (void)viewDidLoad {
    ...
    //加入關注Button
    [_focusBtn addTarget:self action:@selector(focusButton:) forControlEvents:UIControlEventTouchUpInside];
    //單例模式
...
//follower
-(void)focusButton:(UIButton*)sender{
    //get currentUser
    PFUser *currentUser = [PFUser currentUser];
    //如果沒有加入關注的話，按button則加入關注
    if (!_followBool) {
        
        [_focusBtn setImage:[UIImage imageNamed:@"yesFollow.png"] forState:UIControlStateNormal];
        _followBool = YES;
        PFObject *followObj = [PFObject objectWithClassName:@"Follow"];
        //追隨者(follower)
        [followObj setObject:currentUser forKey:@"follower"];
        //被追隨者(followering)
        [followObj setObject:_selectPhotoObj forKey:@"followering"];
        [followObj saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error){
            if (succeeded) {
                [self getFollowNumbers];
            }
        }];
        ...
        [_focusBtn setImage:[UIImage imageNamed:@"notFollow.png"] forState:UIControlStateNormal];
        _followBool = NO;
        [NSObject removeFolloweringWithUser:currentUser withOtherUser:(PFUser*)_selectPhotoObj
                                completions:^(BOOL completion) {
                                    NSLog(@"ok remove");
                                    [self getFollowNumbers];
                                }];
    }
